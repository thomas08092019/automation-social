...
export class AuthController {
  ...
  async handleOAuthCallback(
    ...
    try {
      ...
      if (!code || !state) {
        throw new Error('Missing authorization code or state parameter');      } // Check if this is a social account connection flow or login flow
      ...
      const isSocialAccountFlow =
        state.includes(':') && !state.startsWith('login-');      if (isSocialAccountFlow) {
        ...
        if (result.success) {
          ...
          return res.redirect(successUrl);        } else {
        ...
        try {
          ...
          if (tokens.access_token && !tokens.error && !tokens.error_code) {
            ...
            const errorMsg =
              ...
              tokens.message ||
              tokens.error ||
              'TikTok token exchange failed';            if (
      ...
      const authResult = await this.authService.socialLogin(socialLoginData);      // Redirect to frontend with tokens (use the same base URL determined earlier)
      const frontendUrl =
  ...
  async initiateOAuth(@Param('provider') provider: string, @Res() res: any) {
    ...
    if (provider === 'google') {
      ...
      const authUrl =
        ...
        `_=${Date.now()}&` + // jQuery-style cache buster        `session_invalidate=true&` +

  ...
  async handleOAuthCallback(
    ...
    try {
      ...
      if (codeAge > 300000) {
        // 5 minutes
        console.warn(
          `⚠️  Authorization code is ${codeAge}ms old - may be expired`,
        );
      }
      ...
      if (!tokenResult.success) {
        console.error(
          `❌ Token exchange failed after ${tokenExchangeTime}ms:`,
      ...
      console.log(
        ...
        tokenResult.accessToken
          ? `${tokenResult.accessToken.substring(0, 10)}...`
          : 'MISSING',
      );
      console.log(
        'Refresh Token:',
        tokenResult.refreshToken ? 'Present' : 'Missing',
      );
      ...
      console.log(
        `Token prefix: ${tokenResult.accessToken.substring(0, 20)}...`,
      );
      ...
      try {
        ...
        const userInfoTime = Date.now() - userInfoStartTime;
        console.error(
          `❌ Failed to fetch platform user info for ${socialPlatform} after ${userInfoTime}ms:`,
          error.message,
        );
      ...
      console.log(
        'Access Token:',
        socialAccountData.accessToken ? 'Present' : 'MISSING',
      );
      ...
      console.log('=== SOCIAL ACCOUNT CREATION RESULT ===');
      console.log('Created account ID:', socialAccount.id);
      console.log('Created account platform:', socialAccount.platform);
      console.log(
        'Created account metadata present:',
        !!socialAccount.metadata,
      );
      console.log(
        'Created account metadata keys:',
        socialAccount.metadata
          ? Object.keys(socialAccount.metadata)
          : 'No metadata',
      );
